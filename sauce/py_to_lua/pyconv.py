...
def prepareVisitor():
    ...
    class LuaNodeVisitor(luast.ASTVisitor):
        def __init__(self):
            self.lines = [PY_HEADER]
            self.indent = 0
        # statements
        def visit_LocalAssign(self, node):
            self.newLine()
            for child in node.targets:
                self.visit_Constant(child)
            self.appendExp(" = ")
        ...
        def visit_Function(self, node):
            self.appendLine("def " + node.name.id + "(")
            for i, child in enumerate(node.args):
                self.visit_Constant(child)
                if i < len(node.args) - 1:
                    self.appendExp(", ")
            ...
            if hasattr(node, "comments"):
                for comment in node.comments:
                    ...
                    if comment.startswith("--[["):
                        ...
                        comment = comment[2:]
                        self.appendExp("# " + comment)
                        
        # handle operations, names, numbers, strings, etc.
        def visit_Constant(self, node):
            print(node)
            if isinstance(node, luastnodes.Name):
            ...
            elif isinstance(node, luastnodes.SubOp):
                self.visit_Constant(node.left)
                self.appendExp(" - ")
                self.visit_Constant(node.right)
            elif isinstance(node, luastnodes.MultOp):
                self.visit_Constant(node.left)
                self.appendExp(" * ")
                self.visit_Constant(node.right)
...
def main():
    ...
    if type == 1:
        ...
        try:
            ...
            from luaparser import astnodes as luastnodes
        except:
        ...
        print(luast.to_pretty_str(lua_tree))
        engine = LuaNodeVisitor()
        engine.visit(lua_tree)
        code = clearEmptyLines("\n".join(engine.lines))
